...
typedef struct {
    ...
    DMA_Channel_TypeDef *rxDMAChannel; // receiver DMA 채널 포인터
    DMA_Channel_TypeDef *txDMAChannel; // transmitter DMA 채널 포인터

    uint32_t rxDMAIrq;
    uint32_t txDMAIrq;

    uint32_t rxDMAPos; // DMA reciever일 때, 버퍼를 읽을 위치를 나타낸다.
    bool txDMAEmpty; // DMA buffer가 비어있을 때 true가 된다.
