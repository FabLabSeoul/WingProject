...
static void evaluateCommand(void)
{
    ...
    switch (currentPortState->cmdMSP) {
		...
		case MSP_CUBE_MOTOR:
			...
			for (i=0; i < 8; ++i)
			{
				serialize8(g_cube.thrust[ i].power[ 0]);
				serialize8(g_cube.thrust[ i].power[ 1]);
				serialize8(g_cube.thrust[ i].power[ 2]);
			}
			break;
			
		case MSP_CUBE_RESETHEAD:
			CubeResetHeading();
			break;
		
		case MSP_CUBE_PAIRING:
			CubePairing();
			break;
		
		case MSP_CUBE_DEBUG:
			headSerialReply(76);
			serialize32(*(uint32_t*)&g_cube.dir.x);
			serialize32(*(uint32_t*)&g_cube.dir.y);
			serialize32(*(uint32_t*)&g_cube.dir.z);

			serialize32(*(uint32_t*)&g_cube.m_tm.m[0][0]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[0][1]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[0][2]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[0][3]);

			serialize32(*(uint32_t*)&g_cube.m_tm.m[1][0]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[1][1]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[1][2]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[1][3]);

			serialize32(*(uint32_t*)&g_cube.m_tm.m[2][0]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[2][1]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[2][2]);
			serialize32(*(uint32_t*)&g_cube.m_tm.m[2][3]);
		
			serialize32(*(uint32_t*)&g_cube.m_tm.m[3][0]);
